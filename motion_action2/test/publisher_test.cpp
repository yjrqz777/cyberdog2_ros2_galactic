...
public:
  ...
  void Run(char ** argv)
  {
    if (std::atoi(argv[1]) >= 1000) {
      std_msgs::msg::Int16 msg;
      msg.data = std::atoi(argv[1]);
      moiton_result_queue_pub_->publish(msg);
      INFO("MotionPublisher publish queue cmd: %d", msg.data);
    } else {
...
private:
  ...
  rclcpp::Publisher<std_msgs::msg::Int16>::SharedPtr moiton_result_queue_pub_{nullptr};
