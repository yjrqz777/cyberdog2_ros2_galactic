...
ExecutorStairFind::ExecutorStairFind(std::string node_name): ExecutorBase(node_name) //public rclcpp::Node
{
  auto options = rclcpp::NodeOptions().arguments({"--ros-args", "-r", std::string("__node:=") + get_name() + "_client", "--"});
  action_client_node_ = std::make_shared<rclcpp::Node>("_", options);
  executor_ = std::make_shared<rclcpp::executors::MultiThreadedExecutor>();
  executor_->add_node(action_client_node_);

  // target_tracking_action_client_ = rclcpp_action::create_client<mcr_msgs::action::TargetTracking>(action_client_node_, "tracking_target");

  stair_monitor_client_ = action_client_node_->create_client<std_srvs::srv::SetBool>("elevation_mapping_step_monitor");

  // GetBehaviorManager()->RegisterStateCallback(std::bind(&ExecutorStairFind::UpdateBehaviorStatus, this, std::placeholders::_1));

  // callbackgroup_query_visual_obstacle_avoidance_status_ = action_client_node_->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);
  // query_visual_avoidance_status_service_ = action_client_node_->create_service<std_srvs::srv::Trigger>(
  //   "query_visual_obstacle_avoidance_status",
  //   std::bind(&ExecutorStairFind::QueryVisualObstacleAvoidanceStatusCallback, 
  //   this, std::placeholders::_1,std::placeholders::_2), 

  //   rmw_qos_profile_services_default,callbackgroup_query_visual_obstacle_avoidance_status_);
  ...
  //   std::bind(
  //     &ExecutorStairFind::SwitchVisualObstacleAvoidanceCallback, this, std::placeholders::_1,
  ...
  std::string behavior_config = ament_index_cpp::get_package_share_directory(
    "stair_find") + "/config/UwbTracking.toml";
  toml::value value;
...
// void ExecutorStairFind::ResetLifecycles()
// {
//   UpdateFeedback(AlgorithmMGR::Feedback::TASK_PREPARATION_FAILED);
//   DeactivateDepsLifecycleNodes();
// }

// bool ExecutorStairFind::StartupRealsenseData(bool enable)
// {
...
void ExecutorStairFind::ResetAllDeps()
{
  if (stair_detect_) {
    auto request = std::make_shared<std_srvs::srv::SetBool::Request>();
    request->data = false;
...
void ExecutorStairFind::Start(const AlgorithmMGR::Goal::ConstSharedPtr goal)
{
  ...
  //   std::bind(
  //   &ExecutorStairFind::HandleGoalResponseCallback,
  //   this, std::placeholders::_1);

  // send_goal_options.feedback_callback =
  //   std::bind(
  //   &ExecutorStairFind::HandleFeedbackCallback,
  //   this, std::placeholders::_1, std::placeholders::_2);

  // send_goal_options.result_callback =
  //   std::bind(
  //   &ExecutorStairFind::HandleResultCallback,
  //   this, std::placeholders::_1);
...
void ExecutorStairFind::OnCancel(StopTaskSrv::Response::SharedPtr response)
{
...
void ExecutorStairFind::UpdateBehaviorStatus(const BehaviorManager::BehaviorStatus & status)
{
...
void ExecutorStairFind::QueryVisualObstacleAvoidanceStatusCallback(
...
void ExecutorStairFind::HandleFeedbackCallback(
...
void ExecutorStairFind::HandleResultCallback(const TargetTrackingGoalHandle::WrappedResult result)
{
  switch (result.code) {
