...
SkinManagerNode::SkinManagerNode(std::string name): Node(name)
{
  executor_ = std::make_shared<rclcpp::executors::MultiThreadedExecutor>();
  executor_->add_node(this->get_node_base_interface());
  elec_skin_ = std::make_shared<ElecSkin>();
  launch_service_ = this->create_service<std_srvs::srv::SetBool>("enable_elec_skin",std::bind(&SkinManagerNode::StartSkinCallback, this,std::placeholders::_1, std::placeholders::_2));
  skin_mode_service_ = this->create_service<protocol::srv::ElecSkin>("set_elec_skin",std::bind(&SkinManagerNode::SetModeCallback, this,std::placeholders::_1, std::placeholders::_2));
  std::string elec_skin_config = ament_index_cpp::get_package_share_directory("skin_manager") +"/config/" + "elec_skin.toml";toml::value elec_skin_value;
